//THIS IS THE PYTHON CODE FOR VECTOR VECTOR ADDITION FOR MULTI GPU
//SAURAV RAI
//17558
#include <stdio.h>
#include <cuda.h>
...
/* kernel to execute vector vector addition */
__global__ void vvaddition(int len,float* A,float* B,float *C)
{
...
void result()
{
	printf("Kernel execution done\n");
        ...
        printf("The addition of two vectors is :%f\n",hRes);
...
void routine(void* givendata)
{
	...
	if(data->deviceId==1)
	{
		...
		cudaEventRecord(start,0);
	        vvaddition<<<blocksPerGrid,threadsPerBlock>>>(len,d_a,d_b,d_part_c);
